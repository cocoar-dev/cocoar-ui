...
const REPO_ROOT_DIR = join(ROOT_DIR, '..');
const REFERENCE_DIR = join(REPO_ROOT_DIR, 'docs/reference');
...
function sortComponents(components) {
  return [...components].sort((a, b) => {
    const aKey = (a.selector || a.className || '').toLowerCase();
    const bKey = (b.selector || b.className || '').toLowerCase();
    return aKey.localeCompare(bKey);
  });
}

function generateJsonReference(components) {
...
function generateMarkdownReference(components) {
  const sorted = sortComponents(components);

  const lines = [];
  lines.push('# @cocoar/ui-components — API Reference');
  lines.push('');
  lines.push('This file is auto-generated by `src/scripts/docs/extract-component-api.mjs`.');
  lines.push('');
  lines.push('Install:');
  lines.push('');
  lines.push('```bash');
  lines.push('npm i @cocoar/ui-components @cocoar/ui-tokens');
  lines.push('```');
  lines.push('');
  lines.push('Tokens (global stylesheet):');
  lines.push('');
  lines.push('```css');
  lines.push("@import '@cocoar/ui-tokens/css/all.css';");
  lines.push('```');
  lines.push('');

  for (const comp of sorted) {
    lines.push(`## ${comp.selector || comp.className}`);
    lines.push('');
    if (comp.className) {
      lines.push(`- Class: ${comp.className}`);
    }
    if (comp.selector) {
      lines.push(`- Selector: \`${comp.selector}\``);
    }
    lines.push('');
    ...
    if (!comp.inputs?.length) {
      ...
      for (const input of comp.inputs) {
        ...
        const defaultValue = (input.default ?? '').toString().replace(/\|/g, '\\|');
        const description = (input.description ?? '').toString().replace(/\|/g, '\\|');
        const type = (input.type ?? '').toString().replace(/\|/g, '\\|');
        lines.push(`| ${input.name} | ${type} | ${required} | ${defaultValue} | ${description} |`);
      }
      lines.push('');
    }
    ...
    if (!comp.outputs?.length) {
      ...
      for (const output of comp.outputs) {
        const description = (output.description ?? '').toString().replace(/\|/g, '\\|');
        const type = (output.type ?? '').toString().replace(/\|/g, '\\|');
        lines.push(`| ${output.name} | ${type} | ${description} |`);
      }
      lines.push('');
    }
  }
...
async function main() {
  ...
  const componentFiles = project.getSourceFiles().filter((sf) => {
    ...
    return filePath.includes('/lib/') && filePath.endsWith('.component.ts');
  });
  ...
  const jsonPath = join(REFERENCE_DIR, 'ui-components.api.json');
  writeFileSync(jsonPath, JSON.stringify(generateJsonReference(allComponents), null, 2) + '\n', 'utf-8');

  const mdPath = join(REFERENCE_DIR, 'ui-components.md');
  writeFileSync(mdPath, generateMarkdownReference(allComponents) + '\n', 'utf-8');
