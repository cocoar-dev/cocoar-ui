...
## When to use @cocoar/logging-core
...
```bash
npm install @cocoar/logging-core
```
...
```typescript
import { configureGlobalLogger, getLogger, ConsoleSink } from '@cocoar/logging-core';
...
// Use anywhere in your application or any package
...
```typescript
// Fire-and-forget (default) — call and ignore the return value
logger.info('User action completed');
...
```typescript
// Await for critical logs - waits for all sinks to complete
await logger.fatal('Security breach detected');
await logger.error(error, 'Payment failed for {OrderId}', { OrderId: 789 });

// Only awaits if sinks are async (e.g., HttpSink)
// ConsoleSink is synchronous, so no actual waiting occurs
```
...
Use `{PropertyName}` placeholders in message templates:
...
```typescript
import { LoggerConfiguration, ConsoleSink } from '@cocoar/logging-core';
...
```typescript
import { LoggerConfiguration, ConsoleSink, LogEventLevel } from '@cocoar/logging-core';
...
**Note:** LogEventLevel uses bitwise flags where **lower numbers = higher severity**:
...
```typescript
import { configureGlobalLogger, ConsoleSink, ObservableSink } from '@cocoar/logging-core';
...
```typescript
import { configureGlobalLogger, ConsoleSink } from '@cocoar/logging-core';
import { LogEventLevel } from '@cocoar/logging-core';
...
- Each sink receives events **after** preceding stages (filter, enrich, etc.)
...
configureGlobalLogger((config) =>
  config
    ...
    .fork(p => p
      ...
      .writeTo(errorServiceSink)  // Only errors
    )
    
    .fork(p => p
      .filter(e => e.level <= LogEventLevel.warn)
      .enrich({ destination: 'warning-service' })
      .writeTo(warningServiceSink)  // Warnings and errors
    )
...
```typescript
import { configureGlobalLogger, ConsoleSink, WriteLogLevel } from '@cocoar/logging-core';
...
```typescript
import { Sink, LogEvent } from '@cocoar/logging-core';
...
- **@cocoar/logging-core** — Serilog-style templates, monorepo global logger, async sinks, runtime control
