...
export class CoarOverlayService {
  ...
  private readonly specResolvers =
    inject(COAR_OVERLAY_SPEC_RESOLVERS, { optional: true }) ?? ([] as const satisfies readonly OverlaySpecResolver[]);
  ...
  private attach<TInputs>(spec: ResolvedOverlaySpec<TInputs>, inputs: TInputs, options?: OverlayOpenOptions): OverlayRef {
...
class CoarOverlayRef implements OverlayRef {
  ...
  constructor(
    ...
    this.restoreFocusTarget = (typeof document !== 'undefined' ? document.activeElement : null) ?? null;
  ...
  getRoot(): CoarOverlayRef {
    let current: CoarOverlayRef = this;
  ...
  updatePosition(): void {
    ...
    this.schedule(() => {
      ...
      const attachment = this.spec.attachment;
      const boundaryRect =
        attachment.strategy === 'parent'
          ? getContainerRect(attachment.container)
          : undefined;
  ...
  private applySize(): void {
    ...
    const resolveMax = (value: number | 'viewport' | undefined, viewportSize: number): number | null => {
  ...
  private installRepositionTriggers(): void {
    ...
    const scrollParents = this.spec.anchor.kind === 'element'
      ? getScrollParents(this.spec.anchor.element)
      : [window];
  ...
  private renderContent<TInputs>(content: ContentSpec<TInputs>, host: HTMLElement, inputs: TInputs): () => void {
    switch (content.kind) {
      case 'text': {
        const text = (inputs as { text: string } | null | undefined)?.text;
        if (typeof text !== 'string') {
